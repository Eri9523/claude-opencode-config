...
def main() -> None:
    ...
    try:
        ...
        print("  You can use scripts/source-to-markdown.py as a lightweight fallback instead.", file=sys.stderr)
        raise SystemExit(2)
    ...
    if args.output:
        ...
        print(
            f"✓ MarkItDown wrote {out} ({len(markdown)} characters). "
            "Please continue cleaning up noise and fill in extraction-notes.md.",
            file=sys.stderr,
        )
    else:
