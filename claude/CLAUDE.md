...
- Load `commit-series` for `/commit` and commit preparation; prefer small, coherent commits with their tests and relevant docs. Commit only when Sergio explicitly asks.
...
For frontend work, apply Nielsen's usability heuristics, relevant Laws of UX, and the project's design guidance. Treat laws as context-sensitive heuristics, not absolute rules.
...
<!-- gentle-ai:agent-teams -->
# Agent Teams Lite — Orchestrator Instructions

These rules govern Claude Code orchestration and delegated direct work.
...
- Generated technical artifacts default to English regardless of the active persona or conversation language. This includes specs, designs, tasks, code comments, UI copy, tests, fixtures, and delegated phase outputs.
...
These rules select execution topology, not the implementation method. Implementation runs as **direct inline** or **delegated direct**; size, file count, or risk alone never forces a heavier route.
...
Keep one writer and a short synthesized handoff. Delegation at mapping, write, preparation, and broad-research boundaries remains direct implementation work.
...
<!-- gentle-ai:model-assignments -->
## Model Assignments

Every Claude Agent call includes `model`. Use `sonnet` for generic delegated work and Judgment Day roles; substitute an available equivalent when needed.
<!-- /gentle-ai:model-assignments -->
...
- While it is disabled, keep implementing organically through direct inline or delegated direct work: do not start reviews, retry, or reactivate it.
