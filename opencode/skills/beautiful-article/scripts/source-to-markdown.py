...
def _hint(pkg: str) -> str:
    return f"  (missing {pkg}, install with: pip install {pkg})"
...
def from_pdf(path: str) -> str:
    try:
        ...
        try:
            ...
            print(_hint("pdfplumber or pdfminer.six"), file=sys.stderr)
            raise SystemExit(2)
    out = []
...
def from_url(url: str) -> str:
    try:
        ...
        print("  You can also use the agent's web-fetching ability to get the body text and clean it up afterward.", file=sys.stderr)
        raise SystemExit(2)
...
def main() -> None:
    ...
    if args.output:
        ...
        print(f"✓ Wrote {out} ({len(md)} characters). Please clean up noise manually and fill in extraction-notes.md.", file=sys.stderr)
    else:
        sys.stdout.write(md)
